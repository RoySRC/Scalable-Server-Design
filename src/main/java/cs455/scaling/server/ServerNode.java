...
public class ServerNode {
  ...
  /**
   * Hashmap to map the IP and Port pair of the registered clients to the {@link ServerStatistics} object for keeping
   * track of the number of messages received from the particular client.
   */
   ...
   */
  private BatchTaskManager batch = null;

  /**
   * Selector for creating and listening for activity on channels by selecting a set of keys whose corresponding
   * channels are ready for I/O operations.
   */
  private Selector selector;

  /**
   * ServerSocketChannel for accepting incoming connection from clients. Needs to be registered with the selector
   * with OP_ACCEPT interest such that when there is an activity on the channel, the selector returns a key with the
   * OP_ACCEPT interest flag set. This key can then later be used for performing I/O operation.
   */
  ...
  /**
   * Te following function starts the printer thread responsible for periodically printing the statistics of the
   * server to the screen. Statistics include server throughput, number of active clients, average per-client
   * throughput standard deviation of per-client throughput.
   */
  ...
  public void startServer() throws IOException {
    ...
    serverSocketChannel = ServerSocketChannel.open(); // create input channel
